...
- work_with_animations.js - модуль по созданию динамических анимаций. С помощью данного модуля можно создавать как CSS, так и JS анимацию. Настоятельно рекомендую ознакомится с даннмы модулем. [Описание API по работе с данной абстракцией](####workWithAnimations-API-русская-версия).
...
main.js - точка входа (запуска), в которой собираются исходные алгоритмы игры с помощью каталога [/components_of_algorithms](####Каталог-componentsOfAlgorithms-русская-версия) и утилиты [miniSyncEngine](####miniSyncEngine-API-русская-версия), а также начинается игры при наступлении события "load" у глобального объекта window.
...
#### workWithAnimations API русккая версия
